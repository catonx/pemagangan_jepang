...
<table id="datatable" class="table table-responsive table-striped table-bordered table-hover" width="100%" cellspacing="0">
	...
	<?php
		$no = 1;
		$sql = "SELECT m.id_member, m.nama , count(x.jawaban) as jawaban FROM member m LEFT JOIN (SELECT j.jawaban, j.id_member FROM jawaban j LEFT JOIN soal s ON s.id_soal = j.id_soal WHERE s.jawaban = j.jawaban) AS x ON x.id_member = m.id_member WHERE m.id_member IN (SELECT id_member FROM jawaban) GROUP BY m.id_member";
		$query = $db->query($sql);
