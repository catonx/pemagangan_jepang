...
if(isset($_POST['simpan']) && $_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	if(!empty($passwordlama) && !empty($passwordbaru1) && !empty($passwordbaru2)){
		$getpass=$db->query("select password from user where id_user='{$_SESSION['id_user']}' limit 1")->fetch_assoc();
		if($passwordlama==$getpass['password']){
			if($passwordbaru1==$passwordbaru2){
				$kueri="update user set password='{$passwordbaru1}' where id_user='{$_SESSION['id_user']}' limit 1";
